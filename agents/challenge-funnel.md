...
- Checkout configurado (Eduzz, Hotmart, ASAAS)
...
1. Ler `~/broadcast/signals.json` — filtrar: `campaign_update`, `offer_changed`, `trend_detected`
2. Ler `~/broadcast/mailbox/challenge-funnel.json` — processar mensagens com `read: false`
3. Consultar `~/feedback-loop/results.json` → seções campaigns e offers para dados do challenge ativo
4. Ao concluir fase do challenge: emitir sinal `challenge_phase_complete` e notificar @traffic, @content via mailbox
5. Marcar sinais processados: `bash ~/broadcast/consume-signal.sh {sig_id} @challenge-funnel`
