...
- Voz: Clone ChatterBox do Rudnei
...
2. **Personagem**: Avatar novo ou clone do Rudnei?
...
Workspace: `/Users/rudneisouzamazepa/content-studio/`
...
> **Quando usar:** Toda vez que receber um vídeo bruto gravado pelo Rudnei (selfie/câmera) e for transformá-lo em Reel pronto pra postar. Aplica AUTOMATICAMENTE corte de silêncio + aceleração + legenda OpusClip + b-rolls dinâmicos + flashes punch.

> **Regra de ouro:** Se o pedido contém vídeo do Rudnei + verbos "legenda", "edita", "monta", "reel", "uau", "tira silêncio", "acelera" — disparar pipeline UAU completo. Não perguntar permissão — executar.
...
6. RENDERIZAR FINAL   ffmpeg overlay 12 b-rolls (1.5-2.5s cada)
                      + drawbox flashes brancos em 3 pivots narrativos
                      + ass burn-in
...
Skill `/legendar-video` em `~/.claude/skills/legendar-video.md` documenta o pipeline 1-4 (sem b-rolls). O pipeline UAU completo (com b-rolls + flashes) é responsabilidade desse agente quando o pedido pede "reel pronto" ou "uau".
...
Após render, EXTRAIR 4-6 frames distribuídos no vídeo + USAR Read tool para ver visualmente. Confirmar:
- B-rolls aparecendo nos timestamps corretos
- Legendas sincronizadas com fala
- Acentuação preservada (palavras tipo "INVISÍVEIS", "INCÊNDIO")
- Orientação vertical correta (2160x3840)

Se algo errado: re-render com correção específica.
